...
public class BlackJackHand extends Hand {
    ...
    private int[] getScores(Card card) {
        if(card.faceValue().getValue() > 1) {
            return new int[] {Math.min(card.faceValue().getValue(),10)};
        }else { // Ace
            return new int[] {1,11};
        }
    }
    ...
    private static boolean isAce(Card c) {
        return c.faceValue().getValue() == 1;
    }

    private static boolean isFaceCard(Card c) {
        return c.faceValue().getValue() >= 11 && c.faceValue().getValue() <= 13;
    }
}
